...
target(convertXmlChangeLog: "Converts a full or partial changelog.xml file into a changelog.groovy file") {
  args = args?.split(/\s+/)
  ...
  File targetFile = new File("./migrations", targetName)
  Ant.echo(message: "Converting from ${sourceFile.path} to ${targetFile.path}")
...
setDefaultTarget(convertXmlChangeLog)
